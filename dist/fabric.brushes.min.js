...
0;c<this._points.length;c++){var f=this._points[c].x-this._points[this._count].x;var g=this._points[c].y-this._points[this._count].y;var h=f*f+g*g;2E3>h&&Math.random()>h/2E3&&(b.beginPath(),b.moveTo(a.x+.5*f,a.y+.5*g),b.lineTo(a.x-.5*f,a.y-.5*g),b.stroke())}this._count++},onMouseUp:function(a){0<this._count&&this.convertToImg()},_render:function(){}});e.InkBrush=e.util.createClass(e.BaseBrush,{color:"#000",opacity:1,width:30,_baseWidth:20,_inkAmount:7,_lastPoint:null,_point:null,_range:10,_strokes:null,
initialize:function(a,b){b=b||{};this.canvas=a;this.width=b.width||a.freeDrawingBrush.width;this.color=b.color||a.freeDrawingBrush.color;this.opacity=b.opacity||a.contextTop.globalAlpha;this._point=new e.Point},_render:function(a){var b=this.setPointer(a),c=b.subtract(this._lastPoint),f=b.distanceFrom(this._lastPoint);var g=0;for(a=this._strokes.length;g<a;g++){var h=this._strokes[g];h.update(b,c,f);h.draw()}30<f&&this.drawSplash(b,this._inkAmount)},onMouseDown:function(a){this.canvas.contextTop.globalAlpha=
this.opacity;this._resetTip(a)},onMouseMove:function(a){this.canvas._isCurrentlyDrawing&&this._render(a)},onMouseUp:function(){this.convertToImg();this.canvas.contextTop.globalAlpha=1},drawSplash:function(a,b){var c,f=this.canvas.contextTop,g=e.util.getRandom(12),h=10*b,l=this.color;f.save();for(c=0;c<g;c++){var m=e.util.getRandom(h,1);var k=e.util.getRandom(2*Math.PI);k=new e.Point(a.x+m*Math.sin(k),a.y+m*Math.cos(k));f.fillStyle=l;f.beginPath();f.arc(k.x,k.y,e.util.getRandom(b)/2,0,2*Math.PI,!1);
f.fill()}f.restore()},setPointer:function(a){a=new e.Point(a.x,a.y);this._lastPoint=e.util.object.clone(this._point);return this._point=a},_resetTip:function(a){var b=this.setPointer(a);this._strokes=[];this.size=this.width/5+this._baseWidth;this._range=this.size/2;var c=0;for(a=this.size;c<a;c++)this._strokes[c]=new e.Stroke(this.canvas.contextTop,b,this._range,this.color,this.width,this._inkAmount)}});e.LongfurBrush=e.util.createClass(e.BaseBrush,{color:"#000",opacity:1,width:1,_count:0,_points:[],
...
var f=this._points[a].x-this._points[this._count].x;var g=this._points[a].y-this._points[this._count].y;var h=f*f+g*g;4E3>h&&Math.random()>h/4E3&&(b.beginPath(),b.moveTo(this._points[this._count].x+f*c,this._points[this._count].y+g*c),b.lineTo(this._points[a].x-f*c+2*Math.random(),this._points[a].y-g*c+2*Math.random()),b.stroke())}this._count++},onMouseUp:function(a){0<this._count&&this.convertToImg()},_render:function(){}});e.MarkerBrush=e.util.createClass(e.BaseBrush,{color:"#000",opacity:1,width:30,
_baseWidth:10,_lastPoint:null,_lineWidth:3,_point:null,_size:0,initialize:function(a,b){b=b||{};this.canvas=a;this.width=b.width||a.freeDrawingBrush.width;this.color=b.color||a.freeDrawingBrush.color;this.opacity=b.opacity||a.contextTop.globalAlpha;this.canvas.contextTop.globalAlpha=this.opacity;this._point=new e.Point;this.canvas.contextTop.lineJoin="round";this.canvas.contextTop.lineCap="round"},_render:function(a){var b=this.canvas.contextTop;b.beginPath();var c=0;for(len=this._size/this._lineWidth/
2;c<len;c++){var f=(this._lineWidth-1)*c;b.globalAlpha=.8*this.opacity;b.moveTo(this._lastPoint.x+f,this._lastPoint.y+f);b.lineTo(a.x+f,a.y+f);b.stroke()}this._lastPoint=new e.Point(a.x,a.y)},onMouseDown:function(a){this._lastPoint=a;this.canvas.contextTop.strokeStyle=this.color;this.canvas.contextTop.lineWidth=this._lineWidth;this._size=this.width+this._baseWidth},onMouseMove:function(a){this.canvas._isCurrentlyDrawing&&this._render(a)},onMouseUp:function(){this.canvas.contextTop.globalAlpha=this.opacity;
this.canvas.contextTop.globalAlpha=1}});e.RibbonBrush=e.util.createClass(e.BaseBrush,{color:"#000",opacity:1,width:1,_nrPainters:50,_painters:[],_lastPoint:null,_interval:null,initialize:function(a,b){b=b||{};this.canvas=a;this.width=b.width||a.freeDrawingBrush.width;this.color=b.color||a.freeDrawingBrush.color;this.opacity=b.opacity||1;for(a=0;a<this._nrPainters;a++)this._painters.push({dx:this.canvas.width/2,dy:this.canvas.height/2,ax:0,ay:0,div:.1,ease:.2*Math.random()+.6})},update:function(){for(var a=
