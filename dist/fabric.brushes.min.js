'use strict';(function(e){e.util.trimCanvas=function(a){for(var b=a.getContext("2d"),c=a.width,f=a.height,g=[],h=[],l=b.getImageData(0,0,c,f),m=function(p,q){return p-q},k=0;k<f;k++)for(var n=0;n<c;n++)0<l.data[4*(k*c+n)+3]&&(g.push(n),h.push(k));g.sort(m);h.sort(m);f=g.length-1;c=g[f]-g[0];f=h[f]-h[0];l=b.getImageData(g[0],h[0],c,f);a.width=c;a.height=f;b.putImageData(l,0,0);return{x:g[0],y:h[0]}};e.util.colorValues=function(a){if(a){if("transparent"===a.toLowerCase())return[0,0,0,0];if("#"===a[0])return 7>
